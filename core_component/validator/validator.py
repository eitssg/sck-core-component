...
class Validator:
    ...
    def __init__(
        self, definition_name: str, definition: dict, spec: dict, meta_prefix: str = "_"
    ):
        self.spec = spec
    ...
    def get_validation_results(self) -> tuple[list[dict], list[dict]]:
        return sorted(
            self.validation_errors, key=lambda x: x["Details"]["Key"]
        ), sorted(self.validation_warnings, key=lambda x: x["Details"]["Key"])
    ...
    def __log_validation_error(
        self, spec_key: str, spec: dict, obj_key: str, obj: Any, message: str
    ):
        item: dict = {
    ...
    def __log_validation_warning(
        self, spec_key: str, spec: dict, obj_key: str, obj: dict, message: str
    ):
        item: dict = {
            "Component": self.definition_name,
            "Details": {"Key": obj_key},
    ...
    def __validate(
        ...
        if self.__spec_key("KeyEnum") in spec:
            ...
            enum_values = self.__eval(
                spec_key, spec[self.__spec_key("KeyEnum")], obj_key, key
            )
        ...
        if self.__spec_key("KeyNotEnum") in spec:
            not_enum_values = self.__eval(
                spec_key, spec[self.__spec_key("KeyNotEnum")], obj_key, key
            )
        ...
        if self.__spec_key("KeyRegex") in spec:
            ...
            key_regex = self.__eval(
                spec_key, spec[self.__spec_key("KeyRegex")], obj_key, key
            )
        ...
        if min_length == max_length and (
            len(key) < min_length or len(key) > max_length
        ):
            self.__log_validation_error(
                spec_key,
                spec,
                obj_key,
                obj,
                "Key must be {} characters long, received '{}' ({}) characters".format(
                    min_length, key, len(key)
                ),
            )
            ...
            self.__log_validation_error(
                ...
                obj_key,
                obj,
                "Key must be at least {} characters long, received '{}' ({}) characters".format(
                    min_length, key, len(key)
                ),
            )
            ...
            self.__log_validation_error(
                ...
                obj_key,
                obj,
                "Key must be at most {} characters long, received '{}' ({}) characters".format(
        ...
        if self.__spec_key("Warning") in spec:
            self.__log_validation_warning(
                spec_key, spec, obj_key, obj, spec[self.__spec_key("Warning")]
            )
        ...
        if spec_type == "aws-string":
            ...
            log.error(
                "Unknown type '{}' for spec '{}', failed to validate obj '{}'".format(
                    spec_type, spec_key, obj_key
                )
            )

    def __parse_cardinality(
        self, spec_key: str, cardinality: str, obj_key: str, obj: Any
    ) -> tuple[int, int]:
        cardinality = self.__eval(spec_key, cardinality, obj_key, obj)
        match = re.match(
            r"^(?:([0-9]+)|(?:([0-9]+)-([0-9]+))|(?:([0-9]+)\+))$", str(cardinality)
        )
        if not match:
            raise Exception(
                "Invalid cardinality '{}' on spec '{}'".format(cardinality, spec_key)
            )
        ...
        if max < min:
            raise Exception(
                "Invalid min and max bounds on cardinality '{}'".format(cardinality)
            )

        return min, max

    def __parse_dict_cardinality(
        self, spec_key: str, spec: dict, obj_key: str, obj: Any
    ) -> tuple[int, int]:
        if self.__spec_key("KeyCardinality") in spec:
            # Cardinality is specified - use it for both the min and max values
            return self.__parse_cardinality(
                spec_key, spec[self.__spec_key("KeyCardinality")], obj_key, obj
            )

        required = self.__eval(
            spec_key, spec.get(self.__spec_key("Required"), True), obj_key, obj
        )
        configurable = self.__eval(
            spec_key, spec.get(self.__spec_key("Configurable"), True), obj_key, obj
        )
    ...
    def __parse_key_length(
        self, spec_key: str, spec: dict, obj_key: str, obj: Any
    ) -> tuple[int, int]:
        if self.__spec_key("KeyLength") in spec:
            return self.__parse_cardinality(
                spec_key, spec[self.__spec_key("KeyLength")], obj_key, obj
            )
        else:
            return 0, 100000

    def __parse_list_length(
        self, spec_key: str, spec: dict, obj_key: str, obj: Any
    ) -> tuple[int, int]:
        if self.__spec_key("ListLength") in spec:
            return self.__parse_cardinality(
                spec_key, spec[self.__spec_key("ListLength")], obj_key, obj
            )
        else:
            return 0, 100000

    def __parse_string_length(
        self, spec_key: str, spec: dict, obj_key: str, obj: Any
    ) -> tuple[int, int]:
        if self.__spec_key("StringLength") in spec:
            return self.__parse_cardinality(
                spec_key, spec[self.__spec_key("StringLength")], obj_key, obj
            )
        else:
            return 0, 100000
    ...
    def __is_aws_string(self, spec: dict, obj: dict) -> bool:
        return isinstance(obj, str) or (
            isinstance(obj, dict)
            and len(obj) == 1
            and next(iter(obj)) in self.AWS_STRING_FUNCTIONS
        )
    ...
    def __validate_aws_string(self, spec_key: str, spec: dict, obj_key: str, obj: Any):
        ...
        self.__log_validation_error(
            ...
            obj_key,
            obj,
            "Expecting a string or an AWS CloudFormation intrinsic function, received {}".format(
                type(obj).__name__
            ),
        )
    ...
    def __validate_dict(self, spec_key: str, spec: dict, obj_key: str, obj: Any):
        ...
        for child_spec_key in sorted(spec):
            ...
            validated_keys += self.__validate_dict_item(
                fq_spec_key, spec[child_spec_key], obj_key, obj
            )
    ...
    def __validate_dict_item(
        ...
        if self.__spec_key("KeyEnum") in spec:
            ...
            keys = [
                k
                for k in obj
                if re.match(r"^({})$".format(spec[self.__spec_key("KeyRegex")]), k)
            ]
        else:
        ...
        cardinality_min, cardinality_max = self.__parse_dict_cardinality(
            spec_key, spec, obj_key, obj
        )
        if len(keys) < cardinality_min or len(keys) > cardinality_max:
            if len(keys) == 0 and cardinality_min == 1 and cardinality_max == 1:
                self.__log_validation_error(
                    spec_key, spec, fq_obj_key, obj, "Missing required property"
                )
                ...
                self.__log_validation_error(
                    ...
                    fq_obj_key,
                    obj,
                    "Found {} matching keys, expecting {}".format(
                        len(keys), cardinality_min
                    ),
                )
                ...
                self.__log_validation_error(
                    ...
                    fq_obj_key,
                    obj,
                    "Found {} matching keys, expecting at least {}".format(
                        len(keys), cardinality_min
                    ),
                )
                ...
                self.__log_validation_error(
                    ...
                    fq_obj_key,
                    obj,
                    "Found {} matching keys, expecting at most {}".format(
                        len(keys), cardinality_max
                    ),
                )
        else:
            # Validate each key
    ...
    def __is_eval_boolean(self, spec: dict, obj: Any) -> bool:
        return (
            ...
            and len(obj) == 1
            and next(iter(obj)).startswith("Spec::")
        )

    def __validate_eval_boolean(
        self, spec_key: str, spec: dict, obj_key: str, obj: Any
    ) -> bool:
    ...
    def __validate_float(
        ...
        if self.__spec_key("FloatMinValue") in spec:
            min_value = self.__eval(
                spec_key, spec[self.__spec_key("FloatMinValue")], obj_key, obj
            )
            if value < min_value:
                self.__log_validation_error(
                    spec_key,
        ...
        if self.__spec_key("FloatMaxValue") in spec:
            max_value = self.__eval(
                spec_key, spec[self.__spec_key("FloatMaxValue")], obj_key, obj
            )
            if value > max_value:
                self.__log_validation_error(
                    spec_key,
        ...
        if self.__spec_key("FloatEnum") in spec:
            enum_values = self.__eval(
                spec_key, spec[self.__spec_key("FloatEnum")], obj_key, obj
            )
        ...
        if self.__spec_key("FloatNotEnum") in spec:
            enum_values = self.__eval(
                spec_key, spec[self.__spec_key("FloatNotEnum")], obj_key, obj
            )
            if value not in enum_values:
                self.__log_validation_error(
                    spec_key,
                    ...
                    obj_key,
                    obj,
                    "Value must NOT be one of {}, received '{}'".format(
                        enum_values, value
                    ),
                )
    ...
    def __is_int(self, spec: dict, obj: Any) -> bool:
        if (
            ...
            and spec.get(self.__spec_key("IntTypecast"), True)
        ):
            return True
    ...
    def __validate_int(
        ...
        if self.__spec_key("IntMinValue") in spec:
            min_value = self.__eval(
                spec_key, spec[self.__spec_key("IntMinValue")], obj_key, obj
            )
            if value < min_value:
                self.__log_validation_error(
                    spec_key,
        ...
        if self.__spec_key("IntMaxValue") in spec:
            max_value = self.__eval(
                spec_key, spec[self.__spec_key("IntMaxValue")], obj_key, obj
            )
            if value > max_value:
                self.__log_validation_error(
                    spec_key,
        ...
        if self.__spec_key("IntEnum") in spec:
            enum_values = self.__eval(
                spec_key, spec[self.__spec_key("IntEnum")], obj_key, obj
            )
            if value not in enum_values:
                self.__log_validation_error(
                    spec_key,
        ...
        if self.__spec_key("IntNotEnum") in spec:
            enum_values = self.__eval(
                spec_key, spec[self.__spec_key("IntNotEnum")], obj_key, obj
            )
            if value not in enum_values:
                self.__log_validation_error(
                    spec_key,
        ...
        if self.__spec_key("IntMultipleOf") in spec:
            value_multiple = self.__eval(
                spec_key, spec[self.__spec_key("IntMultipleOf")], obj_key, obj
            )
            if (value % value_multiple) != 0:
                self.__log_validation_error(
                    ...
                    obj_key,
                    obj,
                    "Value must be a multiple of {}, received '{}'".format(
                        value_multiple, value
                    ),
                )
    ...
    def __is_list(self, spec: dict, obj: Any) -> bool:
        return isinstance(obj, list) or spec.get(
            self.__spec_key("ListAllowSingular"), False
        )
    ...
    def __validate_list(
        ...
        if not isinstance(obj, list) and spec.get(
            self.__spec_key("ListAllowSingular"), False
        ):
            obj = [obj]
        ...
        fq_obj_key = obj_key + "." + spec_key.split(".")[-1]
        if (
            len(obj) < min_length or len(obj) > max_length
            ...
            self.__log_validation_error(
                ...
                fq_obj_key,
                obj,
                "List length is {}, but must be at least {}".format(
                    len(obj), min_length
                ),
            )
            ...
            self.__log_validation_error(
                ...
                fq_obj_key,
                obj,
                "List length is {}, but must be at most {}".format(
                    len(obj), max_length
                ),
            )
    ...
    def __validate_multiple(
        ...
        for new_spec in specs:
            ...
            elif new_spec_type == "json-string" and self.__is_json_string(
                new_spec, obj
            ):
                spec_matched = True
        ...
        if not spec_matched:
            spec_types = ", ".join(
                list({s.get(self.__spec_key("Type"), "dict") for s in specs})
            )
            self.__log_validation_error(
                spec_key,
                spec,
                obj_key,
                obj,
                "Expecting one of [{}], received {}".format(
                    spec_types, type(obj).__name__
                ),
            )
    ...
    def __validate_scalar(
        ...
        if isinstance(obj, str):
            ...
            self.__log_validation_error(
                ...
                obj_key,
                obj,
                "Expecting a scalar type [bool, int, str], received {}".format(
                    type(obj).__name__
                ),
            )

    def __is_string(self, spec: dict, obj: Any) -> bool:
        return isinstance(obj, str) or spec.get(
            self.__spec_key("StringTypecast"), False
        )
    ...
    def __validate_string(
        ...
        if self.__spec_key("StringEnum") in spec:
            # Validate Enum constraint
            enum_values = self.__eval(
                spec_key, spec[self.__spec_key("StringEnum")], obj_key, value
            )
            if value not in enum_values:
                self.__log_validation_error(
                    spec_key,
        ...
        if self.__spec_key("StringNotEnum") in spec:
            ...
            enum_values = self.__eval(
                spec_key, spec[self.__spec_key("StringNotEnum")], obj_key, value
            )
        ...
        if self.__spec_key("StringRegex") in spec:
            # Validate Regex constraint
            value_regex = self.__eval(
                spec_key, spec[self.__spec_key("StringRegex")], obj_key, value
            )
            if not re.match("^({})$".format(value_regex), value):
                self.__log_validation_error(
                    ...
                    obj_key,
                    value,
                    "String must match regex '{}', received '{}'".format(
                        value_regex, value
                    ),
                )

        # Ensure string is the correct length
        min_length, max_length = self.__parse_string_length(
            spec_key, spec, obj_key, value
        )
        if (
            len(value) < min_length or len(value) > max_length
        ) and min_length == max_length:
            self.__log_validation_error(
                spec_key,
                spec,
                obj_key,
                value,
                "String must be {} characters long, received '{}' ({}) characters".format(
                    min_length, value, len(value)
                ),
            )
            ...
            self.__log_validation_error(
                ...
                obj_key,
                value,
                "String must be at least {} characters long, received '{}' ({}) characters".format(
                    min_length, value, len(value)
                ),
            )
            ...
            self.__log_validation_error(
                ...
                obj_key,
                value,
                "String must be at most {} characters long, received '{}' ({}) characters".format(
                    max_length, value, len(value)
                ),
            )

    def __eval_if(
        self, spec_key: str, statement_value: Any, obj_key: str, obj: Any
    ) -> Any:
        # Contents must be a list
        if not isinstance(statement_value, list):
            raise Exception(
                "Expected a list for If statement '{}', received '{}'".format(
                    spec_key, type(statement_value).__name__
                )
            )
        # Contents list must contain 3 items
        if len(statement_value) < 1 or len(statement_value) > 3:
            raise Exception(
                "Expected between 1 and 3 items (statement, true value, false value) for If statement '{}'".format(
                    spec_key
                )
            )
    ...
    def __eval_or(
        ...
        if not isinstance(statement_value, list):
            raise Exception(
                "Expected a list for Or statement '{}', received '{}'".format(
                    spec_key, type(statement_value).__name__
                )
            )
        ...
        if len(statement_value) < 1:
            raise Exception(
                "Expected a list with one or more items for Or statement '{}'".format(
                    spec_key
                )
            )
    ...
    def __eval_and(
        ...
        if not isinstance(statement_value, list):
            raise Exception(
                "Expected a list for And statement '{}', received '{}'".format(
                    spec_key, type(statement_value).__name__
                )
            )
        # Contents list must contain 1 or more items
        if len(statement_value) < 1:
            raise Exception(
                "Expected a list with 1 or more items for And statement '{}'".format(
                    spec_key
                )
            )
    ...
    def __eval_property(
        ...
        if not isinstance(statement_value, list):
            raise Exception(
                "Expected a list for Property statement '{}', received '{}'".format(
                    spec_key, type(statement_value).__name__
                )
            )
        if len(statement_value) < 1 or len(statement_value) > 2:
            raise Exception(
                "Expected a list with 1 or 2 items (property name, property value) for Property statement '{}'".format(
                    spec_key
                )
            )
    ...
    def __eval_switch(
        ...
        if not isinstance(statement_value, list):
            raise Exception(
                "Expected a list for Switch statement '{}', received '{}'".format(
                    spec_key, type(statement_value).__name__
                )
            )
        ...
        for item in statement_value:
            ...
            if len(item) != 2:
                raise Exception(
                    "Expected a list with 2 items (condition, value) for Switch item '{}'".format(
                        spec_key
                    )
                )
